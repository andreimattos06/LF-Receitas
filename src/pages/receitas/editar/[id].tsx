...
export default function EditarReceitas(){
    ...
    const [ingredientes, setIngredientes] = useState<listaIngredientes[]>();
    ...
    const [msg_erro, setMsgErro] = useState("")
    


    useEffect(() => {
        const array_ingred = receita?.ingredientes?.split(";")
        const array_set_ingred = array_ingred?.map((e) => {
            let array = e.split("#")
            return( {quantidade: array[0], unidade: array[1], nome: array[2]} )
            
        })
        setIngredientes(array_set_ingred)

        setAutor(receita?.autor)

        setModoPreparo((receita?.modo_preparo?.split(";")))
        setNomeReceita(receita?.nome)
        setTempoPreparo(receita?.tempo_preparo.toString())
        setPorcao(receita?.porcoes)
    ...
    function validarKey() {
        axios.post(`http://localhost:3333/receita/alterar/${id}`,
        {
            edit_key: edit_key,
            ...
            if (response.data != null){
                ...
                setMsgErro("Chave de Edição Inválida!")
            }
        })
    }
    ...
    async function onSubmitEnviar (event){
        ...
        axios.post('http://localhost:3333/receita/alterarreceita', {
            nome:  nome_receita ,      
            autor : autor,       
                ...
                >
                    <Modal.Header >
                        <div className="text-redBase text-2xl font-medium">
                            <span>Digite a Chave de Edição!</span>                            
                        </div>
                    </Modal.Header>
                    <Modal.Body >  

                            <div className="flex flex-col items-center justify-center font-normal text-justify py-3">
                                <input onChange={(event) => {setEditKey(event.target.value)}} placeholder="Ex: 42969738-446b-4bce-87c6-b1d0c93a11c5" className="border-b-2 text-black border-redBase w-full"></input>
                            </div>
                            <span className="text-redBase">{msg_erro}</span>                         
                                                       
                    </Modal.Body>
                    <Modal.Footer>
                        <button onClick={validarKey}>                        
                            <ButtonAnimate texto={"Enviar!"} left_right={1} icon={undefined} />    
                        </button>          
                    </Modal.Footer>
                </Modal>
            </div>
